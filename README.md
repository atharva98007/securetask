...
## Setup & Run (local, SQLite — fastest path)
...
Full request/response schemas are in Swagger (`/docs`) and `postman_collection.json` (import directly into Postman; set the `base_url` collection variable if not running on `localhost:8000`).

### Auth flow

1. `POST /auth/register` → creates account, returns user profile (no token yet).
2. `POST /auth/login` → returns `{ access_token, refresh_token }`.
3. Send `Authorization: Bearer <access_token>` on subsequent requests.
4. When the access token expires (60 min default), `POST /auth/refresh` with the refresh token (7-day default) to get a new pair — the frontend does this automatically on a 401.
...
- JWTs are signed (HS256) and carry `sub` (user id), `role`, `type` (access/refresh), `iat`, `exp`. `SECRET_KEY` **must** be rotated to a long random value in production (see `.env.example` for a generation command) — it is intentionally left as a placeholder here for local dev.
...
See [`SCALABILITY.md`](./SCALABILITY.md) for how this design extends to higher load and more modules (caching, load balancing, DB scaling, microservice extraction).
